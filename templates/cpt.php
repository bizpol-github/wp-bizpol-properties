<div class="wrap">
	...
	<ul class="nav-tabs">
        ...
        <li class="<?php echo isset($_POST['edit_post']) ? 'active' : '' ?>">
        	<a href="#tab-2">
        	<?php echo isset($_POST['edit_post']) ? 'Edit' : 'Add' ?> Custom Post Type</a>
        </li>
        ...
        <div id="tab-1" class="tab-pane <?php echo !isset($_POST['edit_post']) ? 'active' : '' ?>">
			...
			$options = get_option('bizpol_properties_cpt') ?: array();
				...
				foreach ($options as $option) {
					...
					$archive = isset($option['has_achives']) ? "TRUE" : "FALSE";
					...
					settings_fields('bizpol_properties_plugin_cpt_settings');
					echo '<input type="hidden" name="remove" value="' . $option['post_type'] .'">';
;		        	submit_button('Delete', 'delete small', 'submit', false, 					array(
						'onclick' => 'return confirm("Do you want to delete this post type?")'
					));
        ...
        <div id="tab-2" class="tab-pane <?php echo isset($_POST['edit_post']) ? 'active' : '' ?>">
            <form method="post" action="options.php">
		        <?php
		            settings_fields('bizpol_properties_plugin_cpt_settings');
		            do_settings_sections('bizpol_properties_cpt');
		            submit_button();
		        ?>
    		</form>
        </div>
